...
export const usePlaceOrderStore = create<PlaceOrderState>()(
  persist(
    (set, get) => ({
      ...
      setModalTimer: (timer: (prev: number) => number) =>
        set((state) => ({ modalTimer: timer(state.modalTimer) })),
      ...
      handleQuantityChange: (key, increment) => {
        ...
        const newOrderList = orderList.map((order) =>
          order.key === key && order.stockStatus !== "Out of Stock"
            ? {
                ...order,
                quantity: Math.max(0, order.quantity + (increment ? 1 : -1)),
              }
            : order,
        );
      ...
      handleSelectionChange: (keys) => {
        ...
        const selected = orderList.filter(
          (order) =>
            keys.has(order.key) && order.stockStatus !== "Out of Stock",
        );
        set({
          selectedKeys: keys,
          selectedOrders: selected,
          cartCount: selected.reduce(
