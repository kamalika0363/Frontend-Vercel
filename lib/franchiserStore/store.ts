...
export const useOrderHistoryStore = create<OrderHistoryStore>()(
    persist(
        (set) => ({
            ...
            handleSaveEdit: (editedOrder) => set((state) => ({
                orderList: state.orderList.map(order => order.key === editedOrder.key ? editedOrder : order),
                editModalOrder: null
            })),
            handleConfirmDelete: (order) => set((state) => ({
...
export const useFranchiseStore = create<FranchiseStore>()(
    persist(
        (set) => ({
            ...
            setEditModalFranchise: (franchise) => set({editModalFranchise: franchise}),
            setDeleteModalFranchise: (franchise) => set({deleteModalFranchise: franchise}),
            setSortDescriptor: (descriptor) => set({sortDescriptor: descriptor}),
            handleSaveEdit: (editedFranchise) => set((state) => ({
                franchises: state.franchises.map(franchise =>
                    franchise.key === editedFranchise.key ? editedFranchise : franchise
                ),
                editModalFranchise: null
            })),
            handleConfirmDelete: (franchise) => set((state) => ({
...
export const useActiveProductsStore = create<ActiveProductsStore>()(
    persist(
        (set) => ({
            ...
            handleSaveEdit: (editedProduct) => set((state) => ({
                products: state.products.map(product =>
                    product.sku === editedProduct.sku ? editedProduct : product
                ),
                editModalProduct: null
            })),
            handleConfirmDelete: (product) => set((state) => ({
                products: state.products.filter(p => p.sku !== product.sku),
                deleteModalProduct: null
            })),
            handleStockToggle: (product) => set((state) => ({
